...
def list_message(service, query, user_id, count):
    ...
    #senderを取得
        item1 = next((m for m in message_detail['payload']['headers'] if m['name'] == 'From'), None)
        if "chirogasukii@gmail.com" in item1['value']:

            print("From:" + item1['value'])
    ...
    #件名を取得
        item2 = next((m for m in message_detail['payload']['headers'] if m['name'] == 'Subject'), None)
        if item2['value'] == "千葉県千葉市稲毛区穴川町３１５番地":

            print("Subject:" + item2['value'])
...
def list_message1(service, query, user_id, count):
    messages = []
    try:
        message_ids = (
            service.users()
...
# メイン処理
def get_item(query = "is:unread" , count=1):
    ...
    item = list_message1(service, query, "me", count)
